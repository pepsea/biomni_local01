...
def set_env_model(name: str) -> None:
    ...
    for key in ("HYPO_MODEL", "BIOMNI_LLM"):
    ...
    ENV_PATH.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"✓ .env を更新しました: HYPO_MODEL={name} / BIOMNI_LLM={name}")
...
def main() -> int:
    ...
    print(f"設定中のモデル: {settings.model}")
