...
bash scripts/set-provider.sh both --key sk-ant-... --port 8000

# Ollama だけ
bash scripts/set-provider.sh ollama --model qwen3:14b

# Claude API だけ（GPU 不要・イメージも軽い）
bash scripts/set-provider.sh claude --key sk-ant-... --port 8000
```
...
```bash
ssh -L 8000:localhost:8000 user@server    # 手元のブラウザで http://localhost:8000
```
...
| 症状 | 対処 |
|---|---|
| `address already in use` | `lsof -nP -iTCP:8000 -sTCP:LISTEN` で犯人を特定。`start.sh` は起動前に止めて教えます |
