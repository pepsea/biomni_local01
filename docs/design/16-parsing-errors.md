...
> **訂正（§23）**: この文書の「biomni は 2 回まで差し戻し、それでも直らなければ
> 打ち切る」は**誤り**です。その条件は一度も成立しません（差し戻しは
> `HumanMessage` なのに `AIMessage` を数えており、大文字小文字も合っていない）。
> 実測で 28 回まで回りました。打ち切りは `docs/design/23` で自前に持っています。
> また、いちばん多い原因は §22（context が尽きてタグの規定が落ちる）です。
