...
## Docker のとき: bind マウントの下に置かない

画面にこう出ました。

```
/app/workspace/runs.sqlite3 を開けません（データベースを開けません）
```

`/app/workspace` はコンテナ内のパスですが、その実体は `./workspace` の
**bind マウント**です。リポジトリがネットワークマウントの上にあるなら、
コンテナの中から見ても同じファイルシステムなので、やはり sqlite を開けません。
**ホスト側の置き場所の制約を、コンテナがそのまま引き継ぎます。**

逃げる仕組みは働きますが、逃げ先はコンテナの中（`~/.local/state`）なので、
`docker compose up --build` のたびに履歴が消えます。

ラン履歴を名前付きボリュームに移しました。
...
名前付きボリュームは Docker のローカルストレージにあるので、
リポジトリの置き場所に左右されず、再ビルドでも消えません。

テストで 2 点を縛っています。**bind マウントの下に置かない**ことと、
**名前付きボリュームの下に置く**こと。どちらか片方だけだと戻ってしまいます。
