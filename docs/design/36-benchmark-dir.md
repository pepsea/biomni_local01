...
## Docker では UID を自動で合わせる

```
/app/data/biomni_data/data_lake に permission がありません
```

`/app/...` はコンテナ内のパスです。`./data` は bind マウントなので
所有者はホスト側のまま。コンテナのユーザー（`APP_UID`、既定 **1000**）と
食い違うと書けません。**ホストで `chmod` しても、UID が違えば直りません。**

手順を案内するのはもうやめました。`docker-preflight.sh`（`make update` /
`make docker-up` が必ず通る）が、`.env` の `APP_UID` / `APP_GID` を
`id -u` / `id -g` に合わせます。ビルド引数なので、その後の作り直しで効きます。

```
! APP_UID/APP_GID を 501/20 に合わせました（.env）
      コンテナのユーザーがホストと違うと、bind マウントに書けません。
      ビルド引数なので、この後の作り直しで反映されます。
```

2 回目からは黙ります（`✓ APP_UID=501 / APP_GID=20（ホストと一致）`）。

**root では書き込みません。** `APP_UID=0` にすると、コンテナ側で権限を
落としている（`cap_drop: ALL`、`no-new-privileges`）意味が無くなります。
`useradd --uid 0` もそもそも通りません。その場合は所有者を合わせるか、
一般ユーザーで実行し直すよう案内します。

`data/` `workspace/` の**中身**の所有者がずれている場合は、こちらでは
直せない（`sudo` が要る）ので、`chown` のコマンドを出して止めます。

一般ユーザー（uid 1001）で実際に走らせて、`.env` に `APP_UID=1001` が
書かれること、2 回目は書き換えないことを確認しています。
