...
最初は「取得すればこれも使えます」として選択欄の下に一覧を残しました。
これは**間違い**でした。名前が並んでいる以上、あるものと読めます。
「まだ存在しない deepseek などが出る」と指摘を受けました。

**手元に無いモデルの名前は、どこにも出さないこと。** 一覧ごと外しました。

例外は 1 つだけです。ローカルが 0 件のときは選べるものが何も無いので、
「モデルが 1 件もありません」の案内の中で `ollama pull qwen3:14b` を出します。
そこは名前を出さないと次の一手が書けません。

実ブラウザで、`deepseek` / `gpt-oss` / `qwen2.5-coder` / `qwen3:32b` /
`qwen3:8b` のいずれもページ本文に出ないことを確認しています。
テストでも、選択欄の絞り込みと「列挙しないこと」の両方を縛っています。
